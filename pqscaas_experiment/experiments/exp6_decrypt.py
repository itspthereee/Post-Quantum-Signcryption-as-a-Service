"""
Experiment 6: Recipient Decryption
  X-axis: File size (1KB, 10KB, 100KB, 1MB, 10MB, 100MB)
  Y-axis: Decryption computation cost (ms)

All schemes have O(|M|) AEAD cost. Difference comes from PQ operations:
  - PQSCAAS: ML-DSA Verify + ML-KEM Decap
  - Sinha:   ring sig verify + KEM decap
  - Yu:      LWE verify + dot products
  - Bai:     module-lattice verify
"""
...
def plot(df, out_path):
    ...
    ax.set_ylabel('Decryption computation cost (ms)', fontsize=12)
    ax.set_title('Exp 6: Recipient Decryption Cost',
...
if __name__ == "__main__":
    ...
    df = run()
    df.to_csv(os.path.join(base, 'results', 'exp6_decrypt.csv'), index=False)
    plot(df, os.path.join(base, 'figures', 'exp6_decrypt.png'))
    print(f"\n✓ Exp 6 complete.")
