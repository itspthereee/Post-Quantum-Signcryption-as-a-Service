"""
Experiment 2: Signcryption Server-Side Cost Comparison
  X-axis: File size (1KB, 10KB, 100KB, 1MB, 10MB, 100MB)
  Y-axis: Signcryption computation cost (ms)

PQSCAAS: Server-side signcryption cost ONLY (defers all PQ to optimized server)
Baselines: Client-side signcryption cost (AEAD + PQ operations on client)

Fair comparison: PQSCAAS leverages powerful server for PQ operations vs client-only baselines.
PQSCAAS advantage: Server can optimize PQ operations, apply batching, use parallel processing.
"""
...
def run():
    ...
    # Pre-generate keys outside loop (not counted in timing)
    ...
    for size, label in FILE_SIZES:
        ...
        for trial in range(NUM_TRIALS):
            ...
            # PQSCAAS: Measure ONLY server-side signcryption cost
            desc, _ = pq.phase3_client_encrypt(msg)  # Client AEAD (not measured)
            _, t_server = pq.phase4_server_signcrypt_single(desc, pk_r_kem, sk_u_sig)
            pq_runs.append(t_server)  # Only server signcryption cost

            # Baselines: Client-side signcryption only
        ...
        print(f"  PQSCAAS:   {np.mean(pq_runs):>10.3f} ms")
...
def plot(df, out_path):
    ...
    ax.set_title('Exp 2: Signcryption Cost - Server-Side for PQSCAAS vs Client-Side for Baselines',
...
if __name__ == "__main__":
    ...
    df = run()
    df.to_csv(os.path.join(base, 'results', 'exp2_Signcryption End-to-End Latency.csv'), index=False)
    plot(df, os.path.join(base, 'figures', 'exp2_Signcryption End-to-End Latency.png'))
    print(f"\n✓ Exp 2 complete.")
